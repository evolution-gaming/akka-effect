...
lazy val `actor-tests` = (project in file("actor-tests")
  ...
  settings (libraryDependencies ++= Seq(Akka.testkit % Test)))

lazy val testkit = (project in file("testkit")
  settings (name := "akka-effect-testkit")
  settings commonSettings
  dependsOn actor
  settings (libraryDependencies ++= Seq(Akka.testkit % Test, scalatest % Test)))
...
lazy val persistence = (project in file("persistence")
  ...
  settings commonSettings
  dependsOn (`persistence-api` % "test->test;compile->compile",
...
lazy val eventsourcing = (project in file("eventsourcing")
  ...
  settings (libraryDependencies ++= Seq(Akka.stream, retry)))
...
lazy val cluster = (project in file("cluster")
  ...
  testkit          % "test->test;test->compile",
  `actor-tests`    % "test->test")
  settings (libraryDependencies ++= Seq(Akka.cluster, pureconfig)))

lazy val `cluster-sharding` = (project in file("cluster-sharding")
  settings (name := "akka-effect-cluster-sharding")
  settings commonSettings
  dependsOn (cluster % "test->test;compile->compile",
  persistence        % "test->test;compile->compile")
  settings (libraryDependencies ++= Seq(Akka.`cluster-sharding`)))
