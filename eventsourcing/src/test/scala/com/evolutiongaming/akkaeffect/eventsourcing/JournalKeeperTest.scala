...
class JournalKeeperTest extends AsyncFunSuite with Matchers {
  ...
  private def `save snapshot every n events`[F[_]: Async]: F[Unit] = {
    ...
    val config = JournalKeeper.Config(saveSnapshotPerEvents = 2, saveSnapshotCooldown = 0.millis, deleteOldSnapshots = false)
  ...
  private def `not delete snapshots twice`[F[_]: Async]: F[Unit] = {
    ...
    for {
      ...
      _             <- deferred.get
      actions       <- actions.get
      _              = actions.take(3) shouldEqual List(Action.DeleteSnapshots(criteria), Action.SaveSnapshot(4), Action.SaveSnapshot(6))
    } yield {}
  }
  ...
  private def `delete old events`[F[_]: Async]: F[Unit] = {
    ...
    val config =
      JournalKeeper.Config(saveSnapshotPerEvents = 2, saveSnapshotCooldown = 0.millis, deleteOldSnapshots = false, deleteOldEvents = true)
  ...
  private def `maintain cooldown between snapshots`[F[_]: Async]: F[Unit] = {
    ...
    val config = JournalKeeper.Config(saveSnapshotPerEvents = 2, saveSnapshotCooldown = 100.millis, deleteOldSnapshots = false)
