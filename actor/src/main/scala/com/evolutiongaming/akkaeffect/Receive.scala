...
trait Receive[F[_], -A, B] {
  ...
  /**
    ...
    */
  def apply(msg: A, reply: Reply[F, B], sender: ActorRef): F[Stop]
}
...
object Receive {
  ...
  def const[F[_] : Applicative, A, B](stop: F[Stop]): Receive[F, A, B] = {
    (_: A, _: Reply[F, B], _: ActorRef) => stop
  }
  ...
  implicit class ReceiveOps[F[_], A, B](val self: Receive[F, A, B]) extends AnyVal {
    ...
    def mapK[G[_]](fg: F ~> G, gf: G ~> F): Receive[G, A, B] = {
      (msg: A, reply: Reply[G, B], sender: ActorRef) => {
