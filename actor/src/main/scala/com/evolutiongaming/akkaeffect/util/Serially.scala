...
/** Provides serial access to an internal state.
  *
  * The class differs from [[cats.effect.kernel.Ref]] by the ability to execute an effect and a guarantee that the operations will be
  * executed in the same order these arrived given these were called from the same thread.
  */
